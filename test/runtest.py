...
class SerializationTest(unittest.TestCase):
  ...
  def testSerialize(self):
    ...
    ref0 = quartz.push(0, w)
    ref1 = quartz.push(0, w)
    ref2 = quartz.push(0, w)
    ref3 = quartz.push(0, w)
...
class ObjStoreTest(unittest.TestCase):
  ...
  def testObjStore(self):
    ...
    for data in ["h", "h" * 10000, 0, 0.0]:
      objref = quartz.push(data, w1)
      result = quartz.pull(objref, w1)
...
class SchedulerTest(unittest.TestCase):
  ...
  def testRemoteTask(self):
    ...
    value_after = quartz.pull(objref[0], w)
...
class WorkerTest(unittest.TestCase):
  ...
  def testPushPull(self):
    ...
    for i in range(100):
      value_before = i * 10 ** 6
      objref = quartz.push(value_before, w)
      value_after = quartz.pull(objref, w)
      self.assertEqual(value_before, value_after)
...
class APITest(unittest.TestCase):
  ...
  def testObjRefAliasing(self):
    ...
    self.assertTrue(np.alltrue(quartz.pull(objref[0], w) == np.ones([3, 4, 5])))
  ...
  def testKeywordArgs(self):
    ...
    self.assertEqual(quartz.pull(x), "1 hello")
    x = test_functions.keyword_fct1(1, "hi")
    self.assertEqual(quartz.pull(x), "1 hi")
    x = test_functions.keyword_fct1(1, b="world")
    self.assertEqual(quartz.pull(x), "1 world")

    x = test_functions.keyword_fct2(a="w", b="hi")
    self.assertEqual(quartz.pull(x), "w hi")
    x = test_functions.keyword_fct2(b="hi", a="w")
    self.assertEqual(quartz.pull(x), "w hi")
    x = test_functions.keyword_fct2(a="w")
    self.assertEqual(quartz.pull(x), "w world")
    x = test_functions.keyword_fct2(b="hi")
    self.assertEqual(quartz.pull(x), "hello hi")
    ...
    self.assertEqual(quartz.pull(x), "0 1 w hi")
    x = test_functions.keyword_fct3(0, 1, d="hi", c="w")
    self.assertEqual(quartz.pull(x), "0 1 w hi")
    x = test_functions.keyword_fct3(0, 1, c="w")
    self.assertEqual(quartz.pull(x), "0 1 w world")
    x = test_functions.keyword_fct3(0, 1, d="hi")
    self.assertEqual(quartz.pull(x), "0 1 hello hi")
    x = test_functions.keyword_fct3(0, 1)
    self.assertEqual(quartz.pull(x), "0 1 hello world")
  ...
  def testVariableNumberOfArgs(self):
    ...
    self.assertEqual(quartz.pull(x), "0 1 2")
    x = test_functions.varargs_fct2(0, 1, 2)
    self.assertEqual(quartz.pull(x), "1 2")
...
class ReferenceCountingTest(unittest.TestCase):
  ...
  def testDeallocation(self):
    ...
    time.sleep(0.1)
    objref_val = x.val
    self.assertTrue(quartz.scheduler_info()["reference_counts"][objref_val] == 1)

    del x
    self.assertTrue(quartz.scheduler_info()["reference_counts"][objref_val] == -1) # -1 indicates deallocated

    y = test_functions.test_alias_h()
    quartz.pull(y)
    time.sleep(0.1)
    objref_val = y.val
    self.assertTrue(quartz.scheduler_info()["reference_counts"][objref_val:(objref_val + 3)] == [1, 0, 0])

    del y
    self.assertTrue(quartz.scheduler_info()["reference_counts"][objref_val:(objref_val + 3)] == [-1, -1, -1])
