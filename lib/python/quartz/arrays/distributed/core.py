...
class DistArray(object):
  ...
  def assemble(self):
    ...
    first_block = quartz.pull(self.objrefs[(0,) * self.ndim])
    ...
    for index in np.ndindex(*self.num_blocks):
      ...
      result[[slice(l, u) for (l, u) in zip(lower, upper)]] = quartz.pull(self.objrefs[index])
    return result
...
@quartz.remote([DistArray], [np.ndarray])
def assemble(a):
  return a.assemble()

# TODO(rkn): what should we call this method
@quartz.remote([np.ndarray], [DistArray])
def numpy_to_dist(a):
  result = DistArray(a.shape)
  for index in np.ndindex(*result.num_blocks):
    lower = DistArray.compute_block_lower(index, a.shape)
    upper = DistArray.compute_block_upper(index, a.shape)
    result.objrefs[index] = quartz.push(a[[slice(l, u) for (l, u) in zip(lower, upper)]])
  return result

@quartz.remote([List[int], str], [DistArray])
...
@quartz.remote([DistArray], [DistArray])
def copy(a):
  result = DistArray(a.shape)
  for index in np.ndindex(*result.num_blocks):
...
@quartz.remote([int, int, str], [DistArray])
...
@quartz.remote([DistArray, List[int]], [DistArray])
