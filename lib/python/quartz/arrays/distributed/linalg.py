...
@quartz.remote([DistArray], [DistArray, np.ndarray])
def tsqr(a):
  """
  arguments:
  ...
  Suppose that
    ...
    K == min(M, N)
  return values:
    q: DistArray, if q_full = quartz.context.pull(DistArray, q).assemble(), then
      q_full.shape == (M, K)
      np.allclose(np.dot(q_full.T, q_full), np.eye(K)) == True
    r: np.ndarray, if r_val = quartz.context.pull(np.ndarray, r), then
      r_val.shape == (K, N)
...
@quartz.remote([DistArray], [DistArray, np.ndarray, np.ndarray])
def modified_lu(q):
  """
  ...
  for i in range(b):
    L[i, i] = 1
  U = np.triu(q_work)[:b, :]
  return numpy_to_dist(quartz.push(L)), U, S # TODO(rkn): get rid of push and pull

@quartz.remote([np.ndarray, np.ndarray, np.ndarray, int], [np.ndarray, np.ndarray])
...
@quartz.remote([DistArray], [DistArray, DistArray])
def qr(a):
  ...
  result_dtype = np.linalg.qr(quartz.pull(a.objrefs[0, 0]))[0].dtype.name
  r_res = quartz.pull(zeros([k, n], result_dtype)) # TODO(rkn): It would be preferable not to pull this right after creating it.
  y_res = quartz.pull(zeros([m, k], result_dtype)) # TODO(rkn): It would be preferable not to pull this right after creating it.
  Ts = []
  ...
  for i in range(min(a.num_blocks[0], a.num_blocks[1])): # this differs from the paper, which says "for i in range(a.num_blocks[1])", but that doesn't seem to make any sense when a.num_blocks[1] > a.num_blocks[0]
    ...
    if a.shape[0] > a.shape[1]:
      ...
      R_shape = quartz.pull(ra.shape(R))
