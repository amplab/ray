...
@quartz.remote([int, int, int, str], [np.ndarray])
...
@quartz.remote([np.ndarray, List[int], List[int]], [np.ndarray])
...
@quartz.remote([np.ndarray, str], [np.ndarray])
def copy(a, order="K"):
...
@quartz.remote([np.ndarray, List[int]], [np.ndarray])
...
@quartz.remote([int, np.ndarray], [np.ndarray])
def sum(axis, *xs):
  return np.sum(xs, axis=axis)

@quartz.remote([np.ndarray], [tuple])
def shape(a):
  return np.shape(a)
