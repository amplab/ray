...
class Worker(object):
  ...
  def put_object(self, objref, value):
    ...
    if pynumbuf.serializable(value):
      quartz.lib.put_arrow(self.handle, objref, value)
    else:
      ...
      quartz.lib.put_object(self.handle, objref, object_capsule, contained_objrefs)
  ...
  def get_object(self, objref):
    ...
    """
    if quartz.lib.is_arrow(self.handle, objref):
      return quartz.lib.get_arrow(self.handle, objref)
    else:
      object_capsule = quartz.lib.get_object(self.handle, objref)
  ...
  def alias_objrefs(self, alias_objref, target_objref):
    ...
    quartz.lib.alias_objrefs(self.handle, alias_objref, target_objref)
  ...
  def register_function(self, function):
    ...
    quartz.lib.register_function(self.handle, function.func_name, len(function.return_types))
  ...
  def submit_task(self, func_name, args):
    ...
    objrefs = quartz.lib.submit_task(self.handle, task_capsule)
    return objrefs
...
def scheduler_info(worker=global_worker):
  return quartz.lib.scheduler_info(worker.handle);
...
def connect(scheduler_addr, objstore_addr, worker_addr, worker=global_worker):
  if hasattr(worker, "handle"):
    del worker.handle
  worker.handle = quartz.lib.create_worker(scheduler_addr, objstore_addr, worker_addr)

def disconnect(worker=global_worker):
  quartz.lib.disconnect(worker.handle)

def pull(objref, worker=global_worker):
  quartz.lib.request_object(worker.handle, objref)
  return worker.get_object(objref)

def push(value, worker=global_worker):
  objref = quartz.lib.get_objref(worker.handle)
  worker.put_object(objref, value)
  return objref

def main_loop(worker=global_worker):
  if not quartz.lib.connected(worker.handle):
    raise Exception("Worker is attempting to enter main_loop but has not been connected yet.")
  quartz.lib.start_worker_service(worker.handle)
  def process_task(task): # wrapping these lines in a function should cause the local variables to go out of scope more quickly, which is useful for inspecting reference counts
    ...
    quartz.lib.notify_task_completed(worker.handle) # notify the scheduler that the task has completed
  while True:
    task = quartz.lib.wait_for_next_task(worker.handle)
    process_task(task)
...
def check_return_values(function, result):
  if len(function.return_types) == 1:
    ...
    for i in range(len(result)):
      if (not isinstance(result[i], function.return_types[i])) and (not isinstance(result[i], quartz.lib.ObjRef)):
...
def check_arguments(function, args):
  ...
  for (i, arg) in enumerate(args):
    ...
    if isinstance(arg, quartz.lib.ObjRef):
...
def store_outputs_in_objstore(objrefs, outputs, worker=global_worker):
  ...
  for i in range(len(objrefs)):
    if isinstance(outputs[i], quartz.lib.ObjRef):
