...
def serialize(worker_capsule, obj):
  ...
  obj_capsule, contained_objrefs = quartz.lib.serialize_object(worker_capsule, primitive_obj) # contained_objrefs is a list of the objrefs contained in obj
  return obj_capsule, contained_objrefs

def deserialize(worker_capsule, capsule):
  primitive_obj = quartz.lib.deserialize_object(worker_capsule, capsule)
  return from_primitive(primitive_obj)

def serialize_task(worker_capsule, func_name, args):
  primitive_args = [(arg if isinstance(arg, quartz.lib.ObjRef) else to_primitive(arg)) for arg in args]
  return quartz.lib.serialize_task(worker_capsule, func_name, primitive_args)

def deserialize_task(worker_capsule, task):
  func_name, primitive_args, return_objrefs = quartz.lib.deserialize_task(worker_capsule, task)
  args = [(arg if isinstance(arg, quartz.lib.ObjRef) else from_primitive(arg)) for arg in primitive_args]
