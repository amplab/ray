...
def parse_args():
    parser = OptionParser(
        ...
        + "<action> can be: launch, destroy, stop, start, get-main, reboot-subordinates")
...
# Write the public and private ip addresses of the main and subordinate nodes to a file. This will be used by cluster.py.
def write_public_and_private_ip_addresses_to_file(main_nodes, subordinate_nodes):
    main_public_ip_address = main_nodes[0].ip_address
    main_private_ip_address = main_nodes[0].private_ip_address
    subordinate_public_ip_addresses = [node.ip_address for node in subordinate_nodes]
    subordinate_private_ip_addresses = [node.private_ip_address for node in subordinate_nodes]
    all_public_ip_addresses = [main_public_ip_address]
    all_public_ip_addresses.extend(subordinate_public_ip_addresses)
    all_private_ip_addresses = [main_private_ip_address]
    all_private_ip_addresses.extend(subordinate_private_ip_addresses)
...
def real_main():
           ...
           opts.main_instance_type in EC2_INSTANCE_TYPES:
            if EC2_INSTANCE_TYPES[opts.instance_type] != \
               EC2_INSTANCE_TYPES[opts.main_instance_type]:
                print("Error: this script currently does not support having a main and subordinates "
    ...
    if action == "launch":
        ...
        write_public_and_private_ip_addresses_to_file(main_nodes, subordinate_nodes)
