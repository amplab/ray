...
void MemorySegmentPool::open_segment(SegmentId segmentid, size_t size) {
  QUARTZ_LOG(QUARTZ_DEBUG, "Opening segmentid " << segmentid << " on object store " << objstoreid_ << " with create_mode_ = " << create_mode_);
  if (segmentid != segments_.size() && create_mode_) {
    QUARTZ_LOG(QUARTZ_FATAL, "Object store " << objstoreid_ << " is attempting to open segmentid " << segmentid << " on the object store, but segments_.size() = " << segments_.size());
  }
  ...
  if (segments_[segmentid].second == SegmentStatusType::CLOSED) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to open segmentid " << segmentid << ", but segments_[segmentid].second == SegmentStatusType::CLOSED.");
  }
...
void MemorySegmentPool::close_segment(SegmentId segmentid) {
  QUARTZ_LOG(QUARTZ_DEBUG, "closing segmentid " << segmentid);
  std::string segment_name = get_segment_name(segmentid);
...
ObjHandle MemorySegmentPool::allocate(size_t size) {
  if (!create_mode_) { // allocate is called only by the object store
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to call allocate, but create_mode_ is false");
  }
...
uint8_t* MemorySegmentPool::get_address(ObjHandle pointer) {
  if (create_mode_ && segments_[pointer.segmentid()].second != SegmentStatusType::OPENED) {
    QUARTZ_LOG(QUARTZ_FATAL, "Object store " << objstoreid_ << " is attempting to call get_address on segmentid " << pointer.segmentid() << ", which has not been opened yet.");
  }
  if (!create_mode_) {
