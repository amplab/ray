...
static int PyObjRef_init(PyObjRef *self, PyObject *args, PyObject *kwds) {
  ...
  std::vector<ObjRef> objrefs;
  objrefs.push_back(self->val);
  QUARTZ_LOG(QUARTZ_REFCOUNT, "In PyObjRef_init, calling increment_reference_count for objref " << objrefs[0]);
  ...
  if (PyBool_Check(val)) {
    ...
    switch (typ) {
      ...
      case NPY_OBJECT: { // FIXME(pcm): Support arbitrary python objects, not only objrefs
        ...
        break;
      default:
        PyErr_SetString(QuartzError, "serialization: numpy datatype not know");
        return -1;
    }
    Py_DECREF(array); // TODO(rkn): is this right?
  } else {
    PyErr_SetString(QuartzError, "serialization: type not know");
    return -1;
  }
  return 0;
}
...
PyObject* deserialize(PyObject* worker_capsule, const Obj& obj, std::vector<ObjRef> &objrefs) {
  if (obj.has_int_data()) {
    ...
    if (array.double_data_size() > 0) { // TODO: handle empty array
      ...
      switch (array.dtype()) {
        ...
        case NPY_INT64: {
          ...
          break;
        default:
          PyErr_SetString(QuartzError, "deserialization: internal error (array type not implemented)");
          return NULL;
      }
    ...
    PyErr_SetString(QuartzError, "deserialization: internal error (type not implemented)");
    return NULL;
  }
}
...
PyObject* serialize_task(PyObject* self, PyObject* args) {
  ...
  if (PyList_Check(arguments)) {
    ...
    PyErr_SetString(QuartzError, "serialize_task: second argument needs to be a list");
    return NULL;
  }
  Worker* worker;
  PyObjectToWorker(worker_capsule, &worker);
  if (objrefs.size() > 0) {
    QUARTZ_LOG(QUARTZ_REFCOUNT, "In serialize_task, calling increment_reference_count for contained objrefs");
...
PyObject* put_object(PyObject* self, PyObject* args) {
  ...
  if (!PyList_Check(contained_objrefs)) {
    QUARTZ_LOG(QUARTZ_FATAL, "The contained_objrefs argument must be a list.")
  }
...
static PyMethodDef QuartzLibMethods[] = {
...
PyMODINIT_FUNC initlibquartzlib(void) {
  PyObject* m;
  PyObjRefType.tp_new = PyType_GenericNew;
  if (PyType_Ready(&PyObjRefType) < 0) {
    return;
  }
  m = Py_InitModule3("libquartzlib", QuartzLibMethods, "Python C Extension for Quartz");
  Py_INCREF(&PyObjRefType);
  PyModule_AddObject(m, "ObjRef", (PyObject *)&PyObjRefType);
  QuartzError = PyErr_NewException("quartz.error", NULL, NULL);
  Py_INCREF(QuartzError);
  PyModule_AddObject(m, "error", QuartzError);
  import_array();
}
