...
private:
  Status upload_data_to(ObjHandle handle, ObjRef objref, ObjStore::Stub& stub);
  ...
  // these fields are for connecting the object store to itself (as a client):
  MessageQueue<ObjRequest> request_obj_queue_;
  MessageQueue<ObjHandle> receive_obj_queue_;
  // these fields are for the object store as a master:
  MessageQueue<ObjRequest> recv_queue_;
  MessageQueue<ObjHandle> send_queue_; // objstore sending a message to itself
