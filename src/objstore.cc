...
void ObjStoreService::pull_data_from(ObjRef objref, ObjStore::Stub& stub) {
  QUARTZ_LOG(QUARTZ_DEBUG, "Objstore " << objstoreid_ << " is beginning to pull objref " << objref);
  ObjChunk chunk;
  ...
  do {
    if (num_bytes + chunk.data().size() > total_size) {
      QUARTZ_LOG(QUARTZ_FATAL, "The reader attempted to stream too many bytes.");
    }
  ...
  if (num_bytes != total_size) {
    QUARTZ_LOG(QUARTZ_FATAL, "Streamed objref " << objref << ", but num_bytes != total_size");
  }
  object_ready(objref, chunk.metadata_offset());
  QUARTZ_LOG(QUARTZ_DEBUG, "finished streaming data, objref was " << objref << " and size was " << num_bytes);
}
...
Status ObjStoreService::StartDelivery(ServerContext* context, const StartDeliveryRequest* request, AckReply* reply) {
  ...
  ObjRef objref = request->objref();
  {
    ...
    if (memory_[objref].second == MemoryStatusType::NOT_PRESENT) {
      ...
      QUARTZ_LOG(QUARTZ_FATAL, "Objstore " << objstoreid_ << " is attempting to get objref " << objref << ", but memory_[objref] == DEALLOCATED.");
    }
    else {
      QUARTZ_LOG(QUARTZ_DEBUG, "Objstore " << objstoreid_ << " already has objref " << objref << " or it is already being shipped, so no need to pull it again.");
      return Status::OK;
    }
...
Status ObjStoreService::StreamObjTo(ServerContext* context, const StreamObjToRequest* request, ServerWriter<ObjChunk>* writer) {
  QUARTZ_LOG(QUARTZ_DEBUG, "begin to stream data from object store " << objstoreid_);
  ObjChunk chunk;
  ObjRef objref = request->objref();
  memory_lock_.lock();
  if (objref >= memory_.size()) {
    QUARTZ_LOG(QUARTZ_FATAL, "Objstore " << objstoreid_ << " is attempting to use objref " << objref << " in StreamObjTo, but this objref is not present in the object store.");
  }
  if (memory_[objref].second != MemoryStatusType::READY) {
    QUARTZ_LOG(QUARTZ_FATAL, "Objstore " << objstoreid_ << " is attempting to stream objref " << objref << ", but memory_[objref].second != MemoryStatusType::READY.");
  }
  ...
  for (size_t i = 0; i < size; i += CHUNK_SIZE) {
    ...
    if (!writer->Write(chunk)) {
      QUARTZ_LOG(QUARTZ_FATAL, "stream connection prematurely closed")
    }
  }
  return Status::OK;
}
...
Status ObjStoreService::NotifyAlias(ServerContext* context, const NotifyAliasRequest* request, AckReply* reply) {
  ...
  QUARTZ_LOG(QUARTZ_DEBUG, "Aliasing objref " << alias_objref << " with objref " << canonical_objref);
  {
    std::lock_guard<std::mutex> memory_lock(memory_lock_);
    if (canonical_objref >= memory_.size()) {
      QUARTZ_LOG(QUARTZ_FATAL, "Attempting to alias objref " << alias_objref << " with objref " << canonical_objref << ", but objref " << canonical_objref << " is not in the objstore.")
    }
    if (memory_[canonical_objref].second == MemoryStatusType::NOT_READY) {
      QUARTZ_LOG(QUARTZ_FATAL, "Attempting to alias objref " << alias_objref << " with objref " << canonical_objref << ", but objref " << canonical_objref << " is not ready yet in the objstore.")
    }
    if (memory_[canonical_objref].second == MemoryStatusType::NOT_PRESENT) {
      QUARTZ_LOG(QUARTZ_FATAL, "Attempting to alias objref " << alias_objref << " with objref " << canonical_objref << ", but objref " << canonical_objref << " is not present in the objstore.")
    }
    if (memory_[canonical_objref].second == MemoryStatusType::DEALLOCATED) {
      QUARTZ_LOG(QUARTZ_FATAL, "Attempting to alias objref " << alias_objref << " with objref " << canonical_objref << ", but objref " << canonical_objref << " has already been deallocated.")
    }
...
Status ObjStoreService::DeallocateObject(ServerContext* context, const DeallocateObjectRequest* request, AckReply* reply) {
  ...
  if (memory_[canonical_objref].second != MemoryStatusType::READY) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to deallocate canonical_objref " << canonical_objref << ", but memory_[canonical_objref].second = " << memory_[canonical_objref].second);
  }
  if (canonical_objref >= memory_.size()) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to deallocate canonical_objref " << canonical_objref << ", but it is not in the objstore.");
  }
  segmentpool_lock_.lock();
...
void ObjStoreService::process_objstore_request(const ObjRequest request) {
  switch (request.type) {
    case ObjRequestType::ALIAS_DONE: {
      ...
      break;
    default: {
        QUARTZ_LOG(QUARTZ_FATAL, "Attempting to process request of type " <<  request.type << ". This code should be unreachable.");
      }
  }
}
  ...
  switch (request.type) {
    ...
    case ObjRequestType::GET: {
        ...
        if (item.second == MemoryStatusType::READY) {
          QUARTZ_LOG(QUARTZ_DEBUG, "Responding to GET request: returning objref " << request.objref);
          ...
          QUARTZ_LOG(QUARTZ_FATAL, "A worker requested objref " << request.objref << ", but memory_[objref].second = " << memory_[request.objref].second);
        }
      }
      break;
...
void ObjStoreService::process_requests() {
  ...
  while (true) {
    ...
    switch (request.type) {
      case ObjRequestType::ALLOC: {
          QUARTZ_LOG(QUARTZ_VERBOSE, "Request (worker " << request.workerid << " to objstore " << objstoreid_ << "): Allocate object with objref " << request.objref << " and size " << request.size);
          process_worker_request(request);
        }
        break;
      case ObjRequestType::GET: {
          QUARTZ_LOG(QUARTZ_VERBOSE, "Request (worker " << request.workerid << " to objstore " << objstoreid_ << "): Get object with objref " << request.objref);
          process_worker_request(request);
        }
        break;
      case ObjRequestType::WORKER_DONE: {
          QUARTZ_LOG(QUARTZ_VERBOSE, "Request (worker " << request.workerid << " to objstore " << objstoreid_ << "): Finalize object with objref " << request.objref);
          process_worker_request(request);
        }
        break;
      case ObjRequestType::ALIAS_DONE: {
...
ObjHandle ObjStoreService::alloc(ObjRef objref, size_t size) {
  ...
  segmentpool_lock_.unlock();
  std::lock_guard<std::mutex> memory_lock(memory_lock_);
  QUARTZ_LOG(QUARTZ_VERBOSE, "Allocating space for objref " << objref << " on object store " << objstoreid_);
  if (memory_[objref].second != MemoryStatusType::NOT_PRESENT && memory_[objref].second != MemoryStatusType::PRE_ALLOCED) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to allocate space for objref " << objref << ", but memory_[objref].second = " << memory_[objref].second);
  }
...
void ObjStoreService::object_ready(ObjRef objref, size_t metadata_offset) {
  {
    ...
    if (item.second != MemoryStatusType::NOT_READY) {
      QUARTZ_LOG(QUARTZ_FATAL, "A worker notified the object store that objref " << objref << " has been written to the object store, but memory_[objref].second != NOT_READY.");
    }
...
void ObjStoreService::start_objstore_service() {
  communicator_thread_ = std::thread([this]() {
    QUARTZ_LOG(QUARTZ_INFO, "started object store communicator server");
    process_requests();
  });
}
...
void start_objstore(const char* scheduler_addr, const char* objstore_addr) {
  ...
  QUARTZ_LOG(QUARTZ_INFO, "object store " << objstore_addr << " connected to scheduler " << scheduler_addr);
...
int main(int argc, char** argv) {
  if (argc != 3) {
    QUARTZ_LOG(QUARTZ_FATAL, "object store: expected two arguments (scheduler ip address and object store ip address)");
    return 1;
  }
