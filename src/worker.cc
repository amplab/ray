...
Status WorkerServiceImpl::ExecuteTask(ServerContext* context, const ExecuteTaskRequest* request, ExecuteTaskReply* reply) {
  ...
  QUARTZ_LOG(QUARTZ_INFO, "invoked task " << request->task().name());
  Task* taskptr = &task_;
...
SubmitTaskReply Worker::submit_task(SubmitTaskRequest* request) {
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to perform submit_task, but connected_ = " << connected_ << ".");
  }
...
void Worker::request_object(ObjRef objref) {
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to perform request_object, but connected_ = " << connected_ << ".");
  }
...
ObjRef Worker::get_objref() {
  ...
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to perform get_objref, but connected_ = " << connected_ << ".");
  }
...
slice Worker::get_object(ObjRef objref) {
  ...
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to perform get_object, but connected_ = " << connected_ << ".");
  }
  ObjRequest request;
...
void Worker::put_object(ObjRef objref, const Obj* obj, std::vector<ObjRef> &contained_objrefs) {
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to perform put_object, but connected_ = " << connected_ << ".");
  }
  std::string data;
  ...
  if (contained_objrefs.size() > 0) {
    QUARTZ_LOG(QUARTZ_REFCOUNT, "In put_object, calling increment_reference_count for contained objrefs");
...
#define CHECK_ARROW_STATUS(s, msg)                              \
  do {                                                          \
    ...
    if (!_s.ok()) {                                             \
      ...
      PyErr_SetString(QuartzError, _errmsg.c_str());            \
      return NULL;                                              \
    }                                                           \
  } while (0);
...
PyObject* Worker::put_arrow(ObjRef objref, PyObject* value) {
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to perform put_arrow, but connected_ = " << connected_ << ".");
  }
  ObjRequest request;
...
PyObject* Worker::get_arrow(ObjRef objref) {
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to perform get_arrow, but connected_ = " << connected_ << ".");
  }
  ObjRequest request;
  request.workerid = workerid_;
  request.type = ObjRequestType::GET;
...
bool Worker::is_arrow(ObjRef objref) {
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to perform is_arrow, but connected_ = " << connected_ << ".");
  }
  ObjRequest request;
  request.workerid = workerid_;
  request.type = ObjRequestType::GET;
...
void Worker::alias_objrefs(ObjRef alias_objref, ObjRef target_objref) {
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to perform alias_objrefs, but connected_ = " << connected_ << ".");
  }
  ClientContext context;
...
void Worker::increment_reference_count(std::vector<ObjRef> &objrefs) {
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_DEBUG, "Attempting to increment_reference_count for objrefs, but connected_ = " << connected_ << " so returning instead.");
    return;
  }
  if (objrefs.size() > 0) {
    ClientContext context;
    ...
    for (int i = 0; i < objrefs.size(); ++i) {
      QUARTZ_LOG(QUARTZ_REFCOUNT, "Incrementing reference count for objref " << objrefs[i]);
...
void Worker::decrement_reference_count(std::vector<ObjRef> &objrefs) {
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_DEBUG, "Attempting to decrement_reference_count, but connected_ = " << connected_ << " so returning instead.");
    return;
  }
  if (objrefs.size() > 0) {
    ClientContext context;
    ...
    for (int i = 0; i < objrefs.size(); ++i) {
      QUARTZ_LOG(QUARTZ_REFCOUNT, "Decrementing reference count for objref " << objrefs[i]);
      request.add_objref(objrefs[i]);
    }
    AckReply reply;
...
void Worker::register_function(const std::string& name, size_t num_return_vals) {
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to perform register_function, but connected_ = " << connected_ << ".");
  }
  ClientContext context;
...
void Worker::notify_task_completed() {
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to perform notify_task_completed, but connected_ = " << connected_ << ".");
  }
  ClientContext context;
...
void Worker::scheduler_info(ClientContext &context, SchedulerInfoRequest &request, SchedulerInfoReply &reply) {
  if (!connected_) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to get scheduler info, but connected_ = " << connected_ << ".");
  }
...
void Worker::start_worker_service() {
  ...
  worker_server_thread_ = std::thread([service_addr]() {
    ...
    QUARTZ_LOG(QUARTZ_INFO, "worker server listening on " << service_address);
    server->Wait();
  });
}
