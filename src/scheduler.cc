...
Status SchedulerService::RemoteCall(ServerContext* context, const RemoteCallRequest* request, RemoteCallReply* reply) {
  Call* task = new Call(request->call());
  // auto task = request->call().New();
  // task->CopyFrom(request->call());
...
// submit_task assumes that computation_graph_lock_ has been acquired.
void SchedulerService::submit_task(TaskId taskid, WorkerId workerid) {
  ...
  request.mutable_call()->CopyFrom(call);
