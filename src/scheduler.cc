...
Status SchedulerService::SubmitTask(ServerContext* context, const SubmitTaskRequest* request, SubmitTaskReply* reply) {
  ...
  if (fntable_.find(task->name()) == fntable_.end()) {
    ...
    QUARTZ_LOG(QUARTZ_FATAL, "The function " << task->name() << " has not been registered by any worker.");
  }
  ...
  }
  {
    ...
    increment_ref_count(result_objrefs); // We increment once so the objrefs don't go out of scope before we reply to the worker that called SubmitTask. The corresponding decrement will happen in submit_task in quartzlib.
    increment_ref_count(result_objrefs); // We increment once so the objrefs don't go out of scope before the task is scheduled on the worker. The corresponding decrement will happen in deserialize_task in quartzlib.
  }
...
Status SchedulerService::RequestObj(ServerContext* context, const RequestObjRequest* request, AckReply* reply) {
  ...
  if (objref >= size) {
    QUARTZ_LOG(QUARTZ_FATAL, "internal error: no object with objref " << objref << " exists");
  }
...
Status SchedulerService::AliasObjRefs(ServerContext* context, const AliasObjRefsRequest* request, AckReply* reply) {
  ...
  QUARTZ_LOG(QUARTZ_ALIAS, "Aliasing objref " << alias_objref << " with objref " << target_objref);
  if (alias_objref == target_objref) {
    QUARTZ_LOG(QUARTZ_FATAL, "internal error: attempting to alias objref " << alias_objref << " with itself.");
  }
  objtable_lock_.lock();
  size_t size = objtable_.size();
  objtable_lock_.unlock();
  if (alias_objref >= size) {
    QUARTZ_LOG(QUARTZ_FATAL, "internal error: no object with objref " << alias_objref << " exists");
  }
  if (target_objref >= size) {
    QUARTZ_LOG(QUARTZ_FATAL, "internal error: no object with objref " << target_objref << " exists");
  }
  {
    std::lock_guard<std::mutex> target_objrefs_lock(target_objrefs_lock_);
    if (target_objrefs_[alias_objref] != UNITIALIZED_ALIAS) {
      QUARTZ_LOG(QUARTZ_FATAL, "internal error: attempting to alias objref " << alias_objref << " with objref " << target_objref << ", but objref " << alias_objref << " has already been aliased with objref " << target_objrefs_[alias_objref]);
    }
...
Status SchedulerService::RegisterWorker(ServerContext* context, const RegisterWorkerRequest* request, RegisterWorkerReply* reply) {
  ...
  QUARTZ_LOG(QUARTZ_INFO, "registered worker with workerid " << workerid);
...
Status SchedulerService::RegisterFunction(ServerContext* context, const RegisterFunctionRequest* request, AckReply* reply) {
  QUARTZ_LOG(QUARTZ_INFO, "register function " << request->fnname() <<  " from workerid " << request->workerid());
...
Status SchedulerService::ObjReady(ServerContext* context, const ObjReadyRequest* request, AckReply* reply) {
  ObjRef objref = request->objref();
  QUARTZ_LOG(QUARTZ_DEBUG, "object " << objref << " ready on store " << request->objstoreid());
...
Status SchedulerService::WorkerReady(ServerContext* context, const WorkerReadyRequest* request, AckReply* reply) {
  QUARTZ_LOG(QUARTZ_INFO, "worker " << request->workerid() << " reported back");
  {
...
Status SchedulerService::IncrementRefCount(ServerContext* context, const IncrementRefCountRequest* request, AckReply* reply) {
  ...
  if (num_objrefs == 0) {
    QUARTZ_LOG(QUARTZ_FATAL, "Scheduler received IncrementRefCountRequest with 0 objrefs.");
  }
...
Status SchedulerService::DecrementRefCount(ServerContext* context, const DecrementRefCountRequest* request, AckReply* reply) {
  int num_objrefs = request->objref_size();
  if (num_objrefs == 0) {
    QUARTZ_LOG(QUARTZ_FATAL, "Scheduler received DecrementRefCountRequest with 0 objrefs.");
  }
  std::vector<ObjRef> objrefs;
  for (int i = 0; i < num_objrefs; ++i) {
    objrefs.push_back(request->objref(i));
...
Status SchedulerService::AddContainedObjRefs(ServerContext* context, const AddContainedObjRefsRequest* request, AckReply* reply) {
  ...
  // if (!is_canonical(objref)) {
    ...
    // QUARTZ_LOG(QUARTZ_FATAL, "Attempting to add contained objrefs for non-canonical objref " << objref);
  // }
  std::lock_guard<std::mutex> contained_objrefs_lock(contained_objrefs_lock_);
  if (contained_objrefs_[objref].size() != 0) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to add contained objrefs for objref " << objref << ", but contained_objrefs_[objref].size() != 0.");
  }
...
void SchedulerService::deliver_object(ObjRef objref, ObjStoreId from, ObjStoreId to) {
  if (from == to) {
    QUARTZ_LOG(QUARTZ_FATAL, "attempting to deliver objref " << objref << " from objstore " << from << " to itself.");
  }
  if (!has_canonical_objref(objref)) {
    QUARTZ_LOG(QUARTZ_FATAL, "attempting to deliver objref " << objref << ", but this objref does not yet have a canonical objref.");
  }
...
void SchedulerService::schedule() {
  ...
  if (scheduling_algorithm_ == SCHEDULING_ALGORITHM_NAIVE) {
    ...
    QUARTZ_LOG(QUARTZ_FATAL, "scheduling algorithm not known");
  }
...
void SchedulerService::assign_task(OperationId operationid, WorkerId workerid) {
  ...
  QUARTZ_LOG(QUARTZ_INFO, "starting to send arguments");
  for (size_t i = 0; i < task.arg_size(); ++i) {
    if (!task.arg(i).has_obj()) {
      ...
      QUARTZ_LOG(QUARTZ_DEBUG, "task contains object ref " << canonical_objref);
...
std::pair<WorkerId, ObjStoreId> SchedulerService::register_worker(const std::string& worker_address, const std::string& objstore_address) {
  QUARTZ_LOG(QUARTZ_INFO, "registering worker " << worker_address << " connected to object store " << objstore_address);
  ...
  if (objstoreid == std::numeric_limits<size_t>::max()) {
    QUARTZ_LOG(QUARTZ_FATAL, "object store with address " << objstore_address << " not yet registered");
  }
  workers_lock_.lock();
...
ObjRef SchedulerService::register_new_object() {
  ...
  if (objtable_size != target_objrefs_size) {
    QUARTZ_LOG(QUARTZ_FATAL, "objtable_ and target_objrefs_ should have the same size, but objtable_.size() = " << objtable_size << " and target_objrefs_.size() = " << target_objrefs_size);
  }
  if (objtable_size != reverse_target_objrefs_size) {
    QUARTZ_LOG(QUARTZ_FATAL, "objtable_ and reverse_target_objrefs_ should have the same size, but objtable_.size() = " << objtable_size << " and reverse_target_objrefs_.size() = " << reverse_target_objrefs_size);
  }
  if (objtable_size != reference_counts_size) {
    QUARTZ_LOG(QUARTZ_FATAL, "objtable_ and reference_counts_ should have the same size, but objtable_.size() = " << objtable_size << " and reference_counts_.size() = " << reference_counts_size);
  }
  if (objtable_size != contained_objrefs_size) {
    QUARTZ_LOG(QUARTZ_FATAL, "objtable_ and contained_objrefs_ should have the same size, but objtable_.size() = " << objtable_size << " and contained_objrefs_.size() = " << contained_objrefs_size);
  }
...
void SchedulerService::add_location(ObjRef canonical_objref, ObjStoreId objstoreid) {
  ...
  if (!is_canonical(canonical_objref)) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to call add_location with a non-canonical objref (objref " << canonical_objref << ")");
  }
  std::lock_guard<std::mutex> objtable_lock(objtable_lock_);
  if (canonical_objref >= objtable_.size()) {
    QUARTZ_LOG(QUARTZ_FATAL, "trying to put an object in the object store that was not registered with the scheduler (objref " << canonical_objref << ")");
  }
  // do a binary search
...
void SchedulerService::add_canonical_objref(ObjRef objref) {
  ...
  if (objref >= target_objrefs_.size()) {
    QUARTZ_LOG(QUARTZ_FATAL, "internal error: attempting to insert objref " << objref << " in target_objrefs_, but target_objrefs_.size() is " << target_objrefs_.size());
  }
  if (target_objrefs_[objref] != UNITIALIZED_ALIAS && target_objrefs_[objref] != objref) {
    QUARTZ_LOG(QUARTZ_FATAL, "internal error: attempting to declare objref " << objref << " as a canonical objref, but target_objrefs_[objref] is already aliased with objref " << target_objrefs_[objref]);
  }
...
ObjStoreId SchedulerService::pick_objstore(ObjRef canonical_objref) {
  std::mt19937 rng;
  if (!is_canonical(canonical_objref)) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to call pick_objstore with a non-canonical objref, (objref " << canonical_objref << ")");
  }
...
bool SchedulerService::is_canonical(ObjRef objref) {
  ...
  if (target_objrefs_[objref] == UNITIALIZED_ALIAS) {
    QUARTZ_LOG(QUARTZ_FATAL, "Attempting to call is_canonical on an objref for which aliasing is not complete or the object is not ready, target_objrefs_[objref] == UNITIALIZED_ALIAS for objref " << objref << ".");
  }
...
void SchedulerService::perform_pulls() {
  ...
  for (int i = 0; i < pull_queue_.size(); ++i) {
    ...
    if (!has_canonical_objref(objref)) {
      QUARTZ_LOG(QUARTZ_ALIAS, "objref " << objref << " does not have a canonical_objref, so continuing");
      continue;
    }
    ObjRef canonical_objref = get_canonical_objref(objref);
    QUARTZ_LOG(QUARTZ_DEBUG, "attempting to pull objref " << pull.second << " with canonical objref " << canonical_objref << " to objstore " << get_store(workerid));
      ...
      if (std::binary_search(workers.begin(), workers.end(), workerid) && can_run(task)) {
        ...
        for (int j = 0; j < task.arg_size(); ++j) {
          if (!task.arg(j).has_obj()) {
            ...
            if (!has_canonical_objref(objref)) {
              QUARTZ_LOG(QUARTZ_FATAL, "no canonical object ref found even though task is ready; that should not be possible!");
            }
            ObjRef canonical_objref = get_canonical_objref(objref);
...
bool SchedulerService::has_canonical_objref(ObjRef objref) {
  ...
  while (true) {
    if (objref_temp >= target_objrefs_.size()) {
      QUARTZ_LOG(QUARTZ_FATAL, "Attempting to index target_objrefs_ with objref " << objref_temp << ", but target_objrefs_.size() = " << target_objrefs_.size());
    }
...
ObjRef SchedulerService::get_canonical_objref(ObjRef objref) {
  ...
  while (true) {
    ...
    if (target_objrefs_[objref_temp] == UNITIALIZED_ALIAS) {
      QUARTZ_LOG(QUARTZ_FATAL, "Attempting to get canonical objref for objref " << objref << ", which aliases, objref " << objref_temp << ", but target_objrefs_[objref_temp] == UNITIALIZED_ALIAS for objref_temp = " << objref_temp << ".");
    }
    if (target_objrefs_[objref_temp] == objref_temp) {
      return objref_temp;
    }
    objref_temp = target_objrefs_[objref_temp];
    QUARTZ_LOG(QUARTZ_ALIAS, "Looping in get_canonical_objref.");
  }
}
...
void SchedulerService::deallocate_object(ObjRef canonical_objref) {
  ...
  // DecrementRefCount).
  QUARTZ_LOG(QUARTZ_REFCOUNT, "Deallocating canonical_objref " << canonical_objref << ".");
  {
    std::lock_guard<std::mutex> objtable_lock(objtable_lock_);
    auto &objstores = objtable_[canonical_objref];
    ...
    for (int i = 0; i < objstores.size(); ++i) {
      ...
      QUARTZ_LOG(QUARTZ_REFCOUNT, "Attempting to deallocate canonical_objref " << canonical_objref << " from objstore " << objstoreid);
...
void SchedulerService::increment_ref_count(std::vector<ObjRef> &objrefs) {
  ...
  for (int i = 0; i < objrefs.size(); ++i) {
    ...
    if (reference_counts_[objref] == DEALLOCATED) {
      QUARTZ_LOG(QUARTZ_FATAL, "Attempting to increment the reference count for objref " << objref << ", but this object appears to have been deallocated already.");
    }
    reference_counts_[objref] += 1;
    QUARTZ_LOG(QUARTZ_REFCOUNT, "Incremented ref count for objref " << objref <<". New reference count is " << reference_counts_[objref]);
  }
}
...
void SchedulerService::decrement_ref_count(std::vector<ObjRef> &objrefs) {
  ...
  for (int i = 0; i < objrefs.size(); ++i) {
    ObjRef objref = objrefs[i];
    if (reference_counts_[objref] == DEALLOCATED) {
      QUARTZ_LOG(QUARTZ_FATAL, "Attempting to decrement the reference count for objref " << objref << ", but this object appears to have been deallocated already.");
    }
    if (reference_counts_[objref] == 0) {
      QUARTZ_LOG(QUARTZ_FATAL, "Attempting to decrement the reference count for objref " << objref << ", but the reference count for this object is already 0.");
    }
    reference_counts_[objref] -= 1;
    QUARTZ_LOG(QUARTZ_REFCOUNT, "Decremented ref count for objref " << objref << ". New reference count is " << reference_counts_[objref]);
    ...
    if (can_deallocate) {
      ...
      if (!is_canonical(canonical_objref)) {
        QUARTZ_LOG(QUARTZ_FATAL, "canonical_objref is not canonical.");
      }
...
void SchedulerService::get_equivalent_objrefs(ObjRef objref, std::vector<ObjRef> &equivalent_objrefs) {
  ...
  while (target_objrefs_[downstream_objref] != downstream_objref && target_objrefs_[downstream_objref] != UNITIALIZED_ALIAS) {
    QUARTZ_LOG(QUARTZ_ALIAS, "Looping in get_equivalent_objrefs");
...
int main(int argc, char** argv) {
  ...
  if (argc < 2) {
    QUARTZ_LOG(QUARTZ_FATAL, "scheduler: expected at least one argument (scheduler ip address)");
    return 1;
  }
  if (argc > 2) {
